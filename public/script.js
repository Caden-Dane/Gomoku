...
(() => {
  ...
  let myIndex = null;
  let board = [];
  // Default starting board size. Once a game starts, the board size is determined
  // by the length of the `board` array sent from the server. The board can grow
  // dynamically when a player scores, so we no longer rely on a fixed constant.
   ...
   */
  function buildBoard(size = null) {
    // Build the game board in the DOM. The board size is determined by the
    // provided `size` parameter or, if not provided, by the current board
    // state. If the board state is empty (e.g., before a game starts), fall back
    // to the default size.
    let boardSize;
    if (size !== null) {
      boardSize = size;
  ...
  function renderBoard() {
    // Render the board by placing stones according to the `board` array. The
    // board may change size dynamically, so iterate over `board.length`.
    if (!Array.isArray(board) || board.length === 0) return;
    const rows = boardContainer.children;
    const size = board.length;
    for (let r = 0; r < size; r++) {
      const rowDiv = rows[r];
      // Guard against cases where the DOM hasn't been updated to match the board
      if (!rowDiv) continue;
  ...
  function handleServerMessage(data) {
    ...
    switch (type) {
      ...
      case 'gameStarted':
        ...
        // Build the board using the size from the server
      ...
      case 'moveMade':
        ...
        board = JSON.parse(JSON.stringify(data.board));
        currentTurn = data.currentTurn;
        // If the board size has changed (e.g., after a player scores and the board is enlarged),
        // rebuild the board DOM so each cell has the correct coordinates and listeners.
        const newSize = board.length;
        if (boardContainer.children.length !== newSize) {
          buildBoard(newSize);
        }
        renderBoard();
        updateTurnIndicator();
      ...
      case 'gameReset':
        board = JSON.parse(JSON.stringify(data.board));
        scores = data.scores;
        // When the game resets, the server also sends the new currentTurn.
        // Update our local turn state accordingly.
        if (typeof data.currentTurn === 'number') {
          currentTurn = data.currentTurn;
        }
        // The board has been reset to its initial size, so rebuild the board DOM.
        buildBoard(board.length);
        renderBoard();
        updateScoreBoard();
        updateTurnIndicator();
